...
"""Hooked Qwen3 model variant for DMX monitoring integration."""
...
try:
...
except Exception:  # pragma: no cover - keep vendored vLLM usable without monitoring
    MonitoringEngine = None  # type: ignore[assignment]

    class HookPoint(nn.Module):  # type: ignore[no-redef]
    ...
    class HookedRootModule:  # type: ignore[no-redef]
        pass
...
class _MonitoringHookedModelMixin:
    """Minimal hook registry glue for the DMX hooked vLLM models."""

    def _init_monitoring_hook_state(self) -> None:
        self.monitoring_engine = None
        self.mod_dict = {}
        self.hook_dict = {}
        self._native_callbacks_registered = False
        self._native_handles = {}
        self._native_enabled_hooks_key = None

    def setup(self) -> None:
        if hasattr(HookedRootModule, "setup"):
            HookedRootModule.setup(self)
            return
    ...
    def get_monitoring_engine(self) -> MonitoringEngine:
        if MonitoringEngine is None:
            raise RuntimeError("monitoring package is unavailable")
        if self.monitoring_engine is None:
            self.monitoring_engine = MonitoringEngine()
        return self.monitoring_engine

    def prepare_monitoring(self, *args, **kwargs):
        if not hasattr(HookedRootModule, "prepare_monitoring"):
            return None
        return HookedRootModule.prepare_monitoring(self, *args, **kwargs)
...
class Qwen3PDecoderLayer(nn.Module):
    def __init__(
        self,
        config: Qwen3Config,
        cache_config: CacheConfig | None = None,
...
class Qwen3PModel(Qwen2Model):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        super().__init__(
            vllm_config=vllm_config,
            prefix=prefix,
            decoder_layer_type=Qwen3PDecoderLayer,
        )
    ...
    def forward(
        ...
        if get_pp_group().is_first_rank:
            ...
            hidden_states = self.hook_embed(hidden_states)
            residual = None
        else:
...
class Qwen3PForCausalLM(
    _MonitoringHookedModelMixin, nn.Module, SupportsLoRA, SupportsPP, SupportsEagle3
):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self._init_monitoring_hook_state()
        self.setup()
